...
class JSNUniformModelForm extends JModelItem
{
	...
	private function _fieldOthers($post, $fieldSettings, $fieldIdentifier, $validate = true)
	{
		if (isset($fieldSettings->options->allowOther) && $fieldSettings->options->allowOther == 1 && isset($post[$fieldIdentifier]) && $post[$fieldIdentifier] == "Others")
		{
			if ($validate)
			{
				return isset($post["fieldOthers"][$fieldIdentifier]) ? $this->_db->quote($post["fieldOthers"][$fieldIdentifier]) : "''";
			}
			else
			{
		...
		else
		{
			if ($validate)
			{
				return isset($post[$fieldIdentifier]) ? $this->_db->quote($post[$fieldIdentifier]) : "''";
			}
			else
			{
