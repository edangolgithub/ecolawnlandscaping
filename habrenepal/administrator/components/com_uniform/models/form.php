...
class JSNUniformModelForm extends JModelAdmin
{
	...
	public function save($data)
	{
		...
		$listButton->form_btn_next_text = !empty($_POST['jsn_form_button']['form_btn_next_text']) ? $_POST['jsn_form_button']['form_btn_next_text'] : JText::_('NEXT');
		$listButton->form_btn_prev_text = !empty($_POST['jsn_form_button']['form_btn_prev_text']) ? $_POST['jsn_form_button']['form_btn_prev_text'] : JText::_('PREV');
		$listButton->form_btn_submit_text = !empty($_POST['jsn_form_button']['form_btn_submit_text']) ? $_POST['jsn_form_button']['form_btn_submit_text'] : JText::_('SUBMIT');
		$listButton->form_btn_reset_text = !empty($_POST['jsn_form_button']['form_btn_reset_text']) ? $_POST['jsn_form_button']['form_btn_reset_text'] : JText::_('RESET');
		$listButton->form_state_btn_reset_text = !empty($_POST['jsn_form_button']['form_state_btn_reset_text']) ? $_POST['jsn_form_button']['form_state_btn_reset_text'] : "No";
		$data['form_settings'] = json_encode($listButton);
	...
	public function saveField($dataFormId, $dataFormLayout)
	{
					...
					foreach ($formFields as $index => $field)
					{
						if (($index < 10 && strtolower($edition) == "free") || strtolower($edition) != "free")
						{
							...
							if ($checkFieldNew == 0 && !empty($table->field_id) && $table->field_type != "static-content")
							{
								...
								$fieldUpdate[] = "ADD `sb_{$table->field_id}` " . JSNUniformHelper::replaceField($table->field_type)." CHARACTER SET utf8 COLLATE utf8_general_ci ";
							}
		...
		if (empty($dataFormId) || !$checkTableSubmission)
		{
			...
			$this->_db->execute();
			$this->_db->setQuery("CREATE TABLE IF NOT EXISTS #__jsn_uniform_submissions_{$formId} ({$fieldSubmission}) DEFAULT CHARSET=utf8");
			$this->_db->execute();
		}
		if (empty($fieldIds))
		{
