...
class JSNFormGenerateHelper
{
	...
	public static function fieldWebsite($data, $dataSumbission)
	{
		...
		$customClass = !empty($settings->options->customClass) ? $settings->options->customClass : "";
	...
	public static function fieldPhone($data, $dataSumbission)
	{
		...
		$html = "<div class=\"control-group {$customClass} {$identify} {$hideField}\"><label for=\"" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "\" class=\"control-label\">" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "{$required}{$instruction}</label><div class=\"controls {$requiredBlank}\">{$inputContent}</div></div>";
		return $html;
	}
	...
	public static function fieldName($data, $dataSumbission)
	{
		...
		$requiredBlank = !empty($settings->options->required) ? 'group-blank-required' : '';
		$instruction = !empty($settings->options->instruction) ? " <i original-title=\"" . htmlentities(JText::_($settings->options->instruction), ENT_QUOTES, "UTF-8") . "\" class=\"icon-question-sign\"></i>" : '';
		$listField = !empty($settings->options->sortableField) ? json_decode($settings->options->sortableField) : array("vtitle", "vfirst", "vmiddle", "vlast");

		$html = "<div class=\"control-group {$customClass} {$identify} {$hideField}\"><label for=\"" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "\" class=\"control-label\">" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "{$required}{$instruction}</label><div id=\"{$data->field_id}\" class=\"controls {$requiredBlank}\">";
		$valueFirstName = '';
		$valueLastName = '';
		$valueMiddle = '';
	...
	public static function fieldFileUpload($data, $dataSumbission)
	{
		...
		$customClass = !empty($settings->options->customClass) ? $settings->options->customClass : "";
		$requiredBlank = !empty($settings->options->required) ? 'blank-required' : '';
		$multiple = !empty($settings->options->multiple) ? 'multiple' : '';
		$required = !empty($settings->options->required) ? '<span class="required">*</span>' : '';
		$instruction = !empty($settings->options->instruction) ? " <i original-title=\"" . htmlentities(JText::_($settings->options->instruction), ENT_QUOTES, "UTF-8") . "\" class=\"icon-question-sign\"></i>" : '';
		$html = "<div class=\"control-group {$customClass} {$identify} {$hideField}\"><label for=\"" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "\" class=\"control-label\">" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "{$required}{$instruction}</label><div class=\"controls {$requiredBlank}\"><input id=\"{$data->field_id}\" class=\"input-file\" name=\"{$data->field_id}[]\" {$multiple} type=\"file\" /></div></div>";
		return $html;
	}
	...
	public static function fieldEmail($data, $dataSumbission)
	{
		...
		$html = "<div class=\"control-group {$customClass} {$identify} {$hideField}\"><label for=\"" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "\" class=\"control-label\">" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "{$required}{$instruction}</label><div class=\"controls\">";
	...
	public static function fieldAddress($data, $dataSumbission)
	{
		...
		$required = !empty($settings->options->required) ? '<span class="required">*</span>' : '';
		$instruction = !empty($settings->options->instruction) ? " <i original-title=\"" . htmlentities(JText::_($settings->options->instruction), ENT_QUOTES, "UTF-8") . "\" class=\"icon-question-sign\"></i>" : '';
		$listField = !empty($settings->options->sortableField) ? json_decode($settings->options->sortableField) : array("vstreetAddress", "vstreetAddress2", "vcity", "vstate", "vcode", "vcountry");

		$html = "<div class=\"control-group {$customClass} jsn-group-field {$identify} {$hideField} \"><label for=\"" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "\" class=\"control-label\">" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "{$required}{$instruction}</label><div id=\"{$data->field_id}\" class=\"controls {$requiredBlank}\">";
		...
		{
			if (isset($settings->options->$val) && $settings->options->$val == 1)
			{
				$sortableField[] = $val;
			}
			switch($val)
			{
				case "vstreetAddress":
					$field[$val] = "<input type=\"text\" value='" . htmlentities($valueStreet, ENT_QUOTES, "UTF-8") . "' name=\"address[{$data->field_id}][street]\" placeholder=\"" . htmlentities(JText::_("Street_Address"), ENT_QUOTES, "UTF-8") . "\" class=\"jsn-input-xxlarge-fluid\" />";
					break;
				case "vstreetAddress2":
					$field[$val] = "<input type=\"text\" value='" . htmlentities($valueLine2, ENT_QUOTES, "UTF-8") . "' name=\"address[{$data->field_id}][line2]\" placeholder=\"" . htmlentities(JText::_("Address_Line_2"), ENT_QUOTES, "UTF-8") . "\" class=\"jsn-input-xxlarge-fluid\" />";
					break;
				case "vcity":
					$field[$val] = "<input value='" . htmlentities($valueCity, ENT_QUOTES, "UTF-8") . "' type=\"text\" name=\"address[{$data->field_id}][city]\" class=\"jsn-input-xlarge-fluid\" placeholder=\"" . htmlentities(JText::_("City"), ENT_QUOTES, "UTF-8") . "\" />";
					break;
				case "vstate":
					$field[$val] = "<input value='" . htmlentities($valueState, ENT_QUOTES, "UTF-8") . "'  name=\"address[{$data->field_id}][state]\" type=\"text\" placeholder=\"" . htmlentities(JText::_("State_Province_Region"), ENT_QUOTES, "UTF-8") . "\" class=\"jsn-input-xlarge-fluid\" />";
					break;
				case "vcode":
					$field[$val] = "<input value='" . htmlentities($valueCode, ENT_QUOTES, "UTF-8") . "'  type=\"text\" name=\"address[{$data->field_id}][code]\" class=\"jsn-input-xlarge-fluid\" placeholder=\"" . htmlentities(JText::_("Postal_Zip_code"), ENT_QUOTES, "UTF-8") . "\" />";
					break;
				case "vcountry":
					$field[$val] = "<select class=\"jsn-input-xlarge-fluid\" name=\"address[{$data->field_id}][country]\">";
	...
	public static function fieldStaticContent($data, $dataSumbission)
	{
		...
		$html = "<div class=\"control-group {$customClass} {$identify} {$hideField} \"><div class=\"controls\">{$value}</div></div>";
		return $html;
	}
}