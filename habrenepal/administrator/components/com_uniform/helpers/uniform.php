	...
	public static function generateHTMLPages($formId, $formName, $formType = "", $topContent = "", $bottomContent = "")
	{
		...
		foreach ($getHeadData['scripts'] as $script => $option)
		{
			...
			if ($script == JSN_UNIFORM_ASSETS_URI . '/js/submissions.js' || $script == JSN_UNIFORM_ASSETS_URI . '/js/submission.js')
			{
				...
				$scripts[JURI::root(true) . '/plugins/system/jsnframework/assets/3rd-party/bootstrap/js/bootstrap.min.js'] = $option;
				...
				if ($i + 1 == count($formPages))
				{
					...
					elseif (!empty($items->form_captcha) && $items->form_captcha == 2)
					{
						...
						$html .= '<div class="control-group">
								...
								<input type="text" id="jsn-captcha" class="jsn-input-small-fluid" name="captcha" placeholder="' . JText::_("JSN_UNIFORM_CAPTCHA") . '">
								</div>
								</div>';
					}
				}
