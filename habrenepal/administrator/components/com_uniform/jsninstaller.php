...
<script type="text/javascript">
	...
	public static function jsnUniformRreFlight($componentInfo)
	{
		...
		if (!empty($componentInfo->version) && version_compare($componentInfo->version, '2.0.1', '<='))
		{
			$db = JFactory::getDbo();
			$prefix = $db->getPrefix();
			$db->setQuery("SHOW COLUMNS FROM #__jsn_uniform_forms");
			...
			if ($dataFields = $db->loadObjectList())
			{
				...
				foreach ($dataFields as $item)
				{
					if (isset($item->field_type) && isset($item->form_id) && isset($item->field_id) && $item->field_type == "file-upload")
					{
							...
							if ($db->loadResult())
							{
								...
								if (isset($listColumnSubmission[$tableName]))
								{
									foreach ($listColumnSubmission[$tableName] as $columnItems)
									{
										if (isset($columnItems->Field) && $columnItems->Field == "sb_" . $item->field_id)
										{
											$query = "ALTER TABLE " . $db->quoteName($tableName) . " CHANGE " . $db->quoteName("sb_" . $item->field_id) . " " . $db->quoteName("sb_" . $item->field_id) . "  LONGTEXT CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL";
											$db->setQuery($query);
											$db->query();
										}
									}
