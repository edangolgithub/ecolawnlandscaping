...
{	
	...
	if($result<=0){
		$query="ALTER TABLE `#__ijoomeradv_menu`  ADD `menudevice` INT(1) NOT NULL DEFAULT '1' COMMENT 'Global:1,Android:2,Iphone:3,Both:4' AFTER `type`";
		$db->setQuery($query);
		$db->query();
	}
	...
	if($result<=0){
		$query="INSERT INTO `#__ijoomeradv_menu` (`id`, `title`, `menutype`, `note`, `type`, `menudevice`, `published`, `access`, `views`, `home`, `ordering`, `requiredField`, `menuoptions` , `itemimage`) VALUES
				...
				(9, 'External URL', 3, '', 'WebLink', 1, 1, 1, 'default.default.web.Web', 0, 1, 1, '', ''),
				(10, 'Login', 3, 'This will lead user to login screen', 'Login', 1, 1, 1, 'default.default.login.Login', 0, 2, 0, '', '')";
		$db->setQuery($query);
		$db->Query();
	}
		...
		<tr>
			<td colspan="2">
				<div style="text-align:center;">
					...
					<br><font color="#105A8D" size="2"><b>Version 1.3.1</b></font></br>
				</div>
			</td>
		</tr>
		<tr>
