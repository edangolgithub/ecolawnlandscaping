...
if(isset($images->images)){
	...
	if ( $totalimage > 0){
		...
		foreach ($images->images as $image)
		{
			...
			<?php } ?>
			<div class="<?php echo $itemClass;?><?php echo $selectMode;?>" id="<?php echo md5($image['image_extid']);?>">
				...
				<div class="item-thumbnail">
					<a class="item_link<?php echo ($showlistTable->image_source_name == 'folder')?' isloading':'';?>" title="<?php echo $image['image_title'];?>">
						<?php
							$baseurl = ($showlistTable->image_source_type == 'external')?'':JURI::root();
						?>
						<?php if ($showlistTable->image_source_name != 'folder') {?>
						<img id="item_id_<?php echo $i; ?>" class="image_img" src="<?php echo $baseurl.$image['image_small'];?>" style="max-height:60px; max-width: 80px;" alt=""/>
						<?php } else {?>

						<img id="item_id_<?php echo $i; ?>" class="image_img" src="<?php echo (!$isWriable)?$baseurl.$image["image_small"]:'';?>" style="max-height:60px; max-width: 80px;" alt=""/>
						<?php } ?>
					</a>
				</div>
				<div class="item-info">
