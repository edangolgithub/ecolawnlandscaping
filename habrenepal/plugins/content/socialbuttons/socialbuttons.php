	...
	public function onContentAfterTitle ( $context , &$row , &$params , $page = 0 ) {
		...
		if ( $context != 'mod_custom.content' ) {
			...
			if ( $this->params->get( 'twitter' ) == 1 ) {
				...
				$html .= '<a rel="nofollow" href="http://twitter.com/share" class="twitter-share-button" data-url="' . $link . '" data-count="horizontal" data-lang="en">Twitter</a><script src="http://platform.twitter.com/widgets.js" type="text/javascript"></script>' ; 
				$html .= '</div>' ;
			} else { 
				$html .= '' ; 
			}			
			...
			if ( $this->params->get( 'linkedin' ) == 1 ) {
				...
				$html .= '<script type="text/javascript" src="http://platform.linkedin.com/in.js"></script><script type="IN/share" data-url="' . $link . '" data-counter="right"></script>' ; 
				$html .= '</div>' ;
			} else {
				$html .= '' ;
			}
