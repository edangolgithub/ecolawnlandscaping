 ...
 * @version $Id: internal_source_joomgallery.php 18801 2012-11-24 07:30:25Z giangnd $
...
class JSNInternalSourceJoomgallery extends JSNImagesSourcesInternal
{
	...
	function getSyncImages($config)
	{
		...
		if (count($result) > 0)
		{
			foreach ($result as $item)
			{
				...
				$imageObj->image_extid 		= $item->id;
				$imageObj->album_extid		= $item->catid;
				$imageObj->image_small 		= $configJoomga->jg_paththumbs.$catPath.$item->imgthumbname;
	...
	function getImages2JSON($config)
	{
