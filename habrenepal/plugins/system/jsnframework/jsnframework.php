...
class PlgSystemJSNFramework extends JPlugin
{
	...
	public function onBeforeRender()
	{
		...
		if (self::$_app->isAdmin())
		{
			...
			{	
				if (self::$_app->input->getVar('format', '') != 'raw' 
					&& self::$_app->input->getVar('option', '') == 'com_poweradmin' 
					&& self::$_app->input->getVar('tmpl', '') != 'component'					
					&& self::$_app->input->getVar('view') != 'update' 
					&& self::$_app->input->getVar('view') != 'changeposition')
				{
					$version = PoweradminHelper::getVersion();
					if (version_compare($version, '2.2.0', '<='))
					{
						self::$_app->enqueueMessage('Current JSN PowerAdmin version is not compatible with Joomla 3.x, you MUST update to latest version before upgrading your Joomla.', 'warning');
					}
				}			
			}
		}
