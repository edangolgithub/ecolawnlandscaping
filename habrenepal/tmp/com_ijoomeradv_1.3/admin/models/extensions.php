...
class IjoomeradvModelExtensions extends JModel {
	var $_data = null;
	var $_total = null;
...
class JInstaller extends JObject {
	...
	function setAdapter($name, $adapter = null) {
		if (!is_object($adapter)) {
			...
			require_once(dirname(__FILE__).DS.'adapters'.DS.'extensions.php');
		...
		if (is_array($files) && count($files) > 0) {
			foreach ($files as $file) {  
				...
				if (!file_exists($filesource)) {
					JError::raiseWarning(1, 'JInstaller::install: '.JText::sprintf('COM_IJOOMERADV_FILE_DOES_NOT_EXIST', $filesource));
					return false;
