...
class ijoomeradvControllerijoomeradv extends JController{
	...
	private function outputJSON($jsonarray){
		...
		if($encryption == 1){
			...
			echo json_encode($jsonarray);exit; // output the JSON encoded string
		}
	}
		...
		if($result){
			...
			foreach($result as $value){
				...
				if($value->name=='IJOOMER_GC_REGISTRATION'){
					switch($value->value){
						case 'jomsocial':
							$jsonarray['configuration']['globalconfig']['defaultAvatar']=JURI::base().'components/com_community/assets/user_thumb.png';
							break;
					}
				}
			}
